...
* [`src/money_clip/`](src/money_clip/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/money_clip/core.cljs): contains the SPA entry point, `init`
* [`test/money_clip/`](test/money_clip/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `money-clip` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/money_clip/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns money-clip.example
  (:require [money-clip.config :as config])
