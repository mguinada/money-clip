...
(deftest put-change-password
  (let [[{email :email} auth-token password] (system/create-user-and-login @system/app)
    ...
    (testing "when the current password is invalid"
      (let [response (PUT @system/app "/user/change-password" {:current_password "invalid-password" :password new-password :password-confirmation new-password} :headers {"Authorization" (str "Token " auth-token)})]
        (is (= 412 (http/status response)) "Serves a 412 HTTP status code")
    ...
    (testing "when the password confirmation does not match the password"
      (let [response (PUT @system/app "/user/change-password" {:current_password password :password new-password :password-confirmation "not-a-match"} :headers {"Authorization" (str "Token " auth-token)})]
        (is (= 412 (http/status response)) "Serves a 412 HTTP status code")
    ...
    (testing "when the current password and new password confirmation are valid"
      (let [response (PUT @system/app "/user/change-password" {:current_password password :password new-password :password-confirmation new-password} :headers {"Authorization" (str "Token " auth-token)})]
        (is (= 200 (http/status response)) "Serves a 200 HTTP status code")
