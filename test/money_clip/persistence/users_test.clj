...
(deftest create-user-test
  (let [user (u/new-user "john.doe@doe.net" "pa66w0rd" "John" "Doe")]
    (testing "when the user's email is not already in use"
      (is (= "john.doe@doe.net" (::u/email (users/create-user @db/db user "pa66w0rd")))))
    (testing "when the user's password and it's confirmation do not match"
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"Passwords don't match" (users/create-user @db/db user "does-not-match"))))
    (testing "when the user's email is already in use"
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"Email already taken" (users/create-user @db/db user "pa66w0rd"))))))
...
(deftest find-user-by-id-test
  (testing "when the user with the given id exists"
    (let [new-user (u/new-user "john.doe@doe.net" "pa66w0rd" "John" "Doe")
          user (users/create-user @db/db new-user "pa66w0rd")]
...
(deftest authenticate-user-test
  (let [user (users/create-user @db/db (u/new-user "john.doe@doe.net" "pa66w0rd" "John" "Doe") "pa66w0rd")
        _ (users/create-user @db/db (u/new-user "jane.doe@doe.net" "pa66w0rd" "John" "Doe" false) "pa66w0rd")]
    (testing "when the email and password are valid credentials"
      (is (= (dissoc user ::u/password) (users/authenticate-user @db/db "john.doe@doe.net" "pa66w0rd"))) "Authenticates the user")
    (testing "when a user with a given email is not found"
      (is (nil? (users/authenticate-user @db/db "james.doe@doe.net" "pa66w0rd"))) "Returns nil")
    ...
    (testing "when the email and password are valid credentials yet the user is not active"
      (is (nil? (users/authenticate-user @db/db "jane.doe@doe.net" "pa66w0rd"))) "Returns nil")))
