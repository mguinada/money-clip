...
(deftest user-bank-accounts-test
  (let [user (u/user 1 "john.doe@doe.net" "John" "Doe")
    ...
    (testing "when there user has bank accounts"
      (let [db (sh/mock users/Users {:find-user-by-id user}
                        bank-accounts/BankAccounts {:find-bank-accounts-by-user [bank-account-1 bank-account-2]})
            handler (ig/init-key :money-clip.handler.bank-accounts/user-bank-accounts {:db db})
            response (handler (-> (mock/request :get "/bank-accounts") (mock/identity (select-keys user [::u/id ::u/email]))))]
        (is (sh/received? db users/find-user-by-id (list (::u/id user))) "Fetches the user")
    ...
    (testing "when there user is not found"
      (let [db (sh/mock users/Users {:find-user-by-id nil}
                        bank-accounts/BankAccounts {:find-bank-accounts-by-user [bank-account-1 bank-account-2]})
            handler (ig/init-key :money-clip.handler.bank-accounts/user-bank-accounts {:db db})
            response (handler (-> (mock/request :get "/bank-accounts") (mock/identity (select-keys user [::u/id ::u/email]))))]
        (is (sh/received? db users/find-user-by-id (list (::u/id user))) "Tries to fetch the user")
        (is (not (sh/received? db bank-accounts/find-bank-accounts-by-user (list user))) "Does not fetch the bank accounts")
        (is (= :ataraxy.response/not-found (first response)) "HTTP response")
        (is (nil? (second response)) "Does not list the bank accounts")))))
...
(deftest user-bank-account-test
  (let [user (u/user 1 "john.doe@doe.net" "John" "Doe")
    ...
    (testing "when there user has a bank account with the given id"
      (let [db (sh/mock users/Users {:find-user-by-id user}
                        bank-accounts/BankAccounts {:find-bank-account-by-user-and-id bank-account})
            handler (ig/init-key :money-clip.handler.bank-accounts/user-bank-account {:db db})
            response (handler (-> (mock/request :get "/bank-accounts/" {:id 1}) (mock/identity (select-keys user [::u/id ::u/email]))))]
        (is (sh/received? db users/find-user-by-id (list (::u/id user))) "Fetches the user")
    ...
    (testing "when there user has not bank account with the given id"
      (let [db (sh/mock users/Users {:find-user-by-id user}
                        bank-accounts/BankAccounts {:find-bank-account-by-user-and-id nil})
            handler (ig/init-key :money-clip.handler.bank-accounts/user-bank-account {:db db})
            response (handler (-> (mock/request :get "/bank-accounts/" {:id 1}) (mock/identity (select-keys user [::u/id ::u/email]))))]
        (is (sh/received? db users/find-user-by-id (list (::u/id user))) "Fetches the user")
        (is (sh/received? db bank-accounts/find-bank-account-by-user-and-id (list user (::ba/id bank-account))) "Fetches the bank account")
    ...
    (testing "when there user is not found"
      (let [db (sh/mock users/Users {:find-user-by-id nil}
            ...
            response (handler (-> (mock/request :get "/bank-accounts/" {:id 1}) (mock/identity (select-keys user [::u/id ::u/email]))))]
        (is (sh/received? db users/find-user-by-id (list (::u/id user))) "Tries to fetch the user")
        (is (not (sh/received? db bank-accounts/find-bank-account-by-user-and-id (list user (::ba/id bank-account)))) "Does not fetch the bank account")
        (is (= :ataraxy.response/not-found (first response)) "HTTP response")))))
